...
// enxuto que o resto do app consome.
const PncpNormalize = (() => {
  const PNCP_APP_URL = 'https://pncp.gov.br/app';
  ...
  function montarLinkPncp(itemPncp) {
    ...
    if (cnpj && ano && sequencial) {
      return `${PNCP_APP_URL}/editais/${cnpj}/${ano}/${sequencial}`;
    }
    if (itemPncp.item_url) {
      return `${PNCP_APP_URL}${itemPncp.item_url.replace('/compras/', '/editais/')}`;
    }
    return PNCP_APP_URL;
  }
  ...
  function normalizarLicitacao(itemPncp, palavraChave) {
    return {
      ...
      dataInicioVigencia: itemPncp.data_inicio_vigencia || null,
